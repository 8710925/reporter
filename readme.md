# Grafana reporter <img style="float: right;" src="https://travis-ci.org/8710925/reporter.svg?branch=master">
...
Get the source files and dependencies:
    ...
    go get github.com/8710925/reporter/...
...
Also see [this issue](https://github.com/8710925/reporter/issues/50) for an example. 
...
A Docker image [is available](https://hub.docker.com/r/8710925/grafana-reporter/). To see available flags:

    docker run 8710925/grafana-reporter --help

To run with default flags, use `--net` to enable Docker to connect to Grafana at `localhost:3000`:

    docker run -p 8686:8686 --net="host" 8710925/grafana-reporter
...
If you also have `Make` and `Docker-compose` installed, you can run a simple local orchestration of Grafana and Grafana-reporter:
     ...
     go get github.com/8710925/reporter/ ...
     cd $GOPATH/src/github.com/8710925/reporter
     make compose-up
...
The unit tests can be run using the go tool:
    ...
    go test -v github.com/8710925/reporter/...

or, the [GoConvey](http://goconvey.co/) webGUI:

    ./bin/goconvey -workDir `pwd`/src/github.com/8710925 -excludedDirs `pwd`/src/github.com/8710925/reporter/tmp/
